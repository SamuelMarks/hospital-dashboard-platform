...
async def test_search_functionality(client: AsyncClient) -> None:
    ...
    token = (
        await client.post(
            "/api/v1/auth/login", data={"username": email, "password": pwd}
        )
    ).json()["access_token"]
    ...
    # 3. Search for "Admit"
    res = await client.get(f"{TEMPLATES_URL}/?search=Admit", headers=headers)
    assert res.status_code == 200
    data = res.json()
    assert len(data) == 1
    assert data[0]["title"] == "Admissions Report"

    # 4. Search for "Metrics" (Case insensitive check)
...
async def test_limit_pagination(client: AsyncClient) -> None:
    ...
    await client.post(
        "/api/v1/auth/register", json={"email": email, "password": "x"}
    )
    token = (
        await client.post(
            "/api/v1/auth/login", data={"username": email, "password": "x"}
        )
    ).json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    res = await client.get(
        f"{TEMPLATES_URL}/?limit=2&category=Logistics", headers=headers
    )
