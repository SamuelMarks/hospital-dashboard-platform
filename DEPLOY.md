...
```bash
# Provision the stack on Azure (creates vm-pulse-query)
~/repos/libscript/libscript.sh provision azure vm-pulse-query rg-analytics-prod eastus ./ pulse-query

# Map the domain to the newly provisioned node
~/repos/libscript/libscript.sh cloud azure dns map-node vm-pulse-query rg-analytics-prod pulse-query.healthplatform.io healthplatform-zone
```
...
```bash
~/repos/libscript/libscript.sh deprovision azure vm-pulse-query rg-analytics-prod eastus ./ pulse-query
```
...
```bash
~/repos/libscript/libscript.sh cloud azure node deploy shared-node rg-analytics-prod ./ pulse-query
```
...
```bash
# Map DNS
~/repos/libscript/libscript.sh cloud azure dns map-node shared-node rg-analytics-prod pulse-query.healthplatform.io healthplatform-zone

# Install dependencies and start the stack on the remote node
~/repos/libscript/libscript.sh cloud azure node exec shared-node rg-analytics-prod "cd pulse-query && sudo ~/libscript/libscript.sh install-deps"
~/repos/libscript/libscript.sh cloud azure node exec shared-node rg-analytics-prod "cd pulse-query && sudo ~/libscript/libscript.sh start"
```
...
```bash
# Unmap DNS
~/repos/libscript/libscript.sh cloud azure dns unmap-node shared-node rg-analytics-prod pulse-query.healthplatform.io healthplatform-zone
...
~/repos/libscript/libscript.sh cloud azure node deploy shared-node rg-analytics-prod ./ pulse-query-alpha

# Deploy Production version
~/repos/libscript/libscript.sh cloud azure node deploy shared-node rg-analytics-prod ./ pulse-query-prod
```
...
```bash
# Map Alpha DNS
~/repos/libscript/libscript.sh cloud azure dns map-node shared-node rg-analytics-prod alpha.pulse-query.healthplatform.io healthplatform-zone
...
For the alpha environment on the shared node, you would edit `~/pulse-query-alpha/libscript.json`:
```json
{
  "domain": "alpha.pulse-query.healthplatform.io"
}
```
...
```bash
~/repos/libscript/libscript.sh cloud azure node exec shared-node rg-analytics-prod "cd pulse-query-alpha && sudo ~/libscript/libscript.sh start"
~/repos/libscript/libscript.sh cloud azure node exec shared-node rg-analytics-prod "cd pulse-query-prod && sudo ~/libscript/libscript.sh start"
```
