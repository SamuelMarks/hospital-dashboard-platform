...
test.describe('Widget Wizard - Custom SQL Flow', () => {
  ...
  test('should create Custom SQL widget without getting stuck on configuration step', async ({
    loggedInPage,
  }) => {
    // 1. Toggle "Edit Mode"
    ...
    await expect(loggedInPage.locator('app-sql-builder .result-container viz-table')).toBeVisible({
      timeout: 15000,
    });
