...
test.describe('Dashboard Layout Management', () => { 
  ...
  test.beforeEach(async ({ loggedInPage }) => { 
    ...
    await loggedInPage.getByTestId('input-name').pressSequentially(`Layout Test ${Date.now()}`, { delay: 50 }); 
  ...
  test('should reorder and resize widgets persistently', async ({ loggedInPage }) => { 
    ...
    await test.step('Drag Widget A to Widget B position', async () => { 
      ...
      const saveReq = loggedInPage.waitForResponse(r =>
    ...
    await test.step('Resize Widget', async () => { 
      ...
      const putPromise = loggedInPage.waitForResponse(resp =>
        resp.url().includes('/widgets/') && resp.request().method() === 'PUT' 
      ); 
