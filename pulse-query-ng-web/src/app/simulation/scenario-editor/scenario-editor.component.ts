...
@Component({ 
  ...
  styles: [`
    ...
    .pane-header { padding: 24px; border-bottom: 1px solid var(--sys-surface-border); } 
    .pane-content { flex: 1; overflow-y: auto; } 
    .action-footer { padding: 16px; border-top: 1px solid var(--sys-surface-border); background-color: var(--sys-surface); } 

    /* Sliders & Inputs */ 
    .capacity-list { display: flex; flex-direction: column; gap: 24px; padding: 8px 0; } 
    .capacity-row { display: flex; flex-direction: column; } 
    .row-label { display: flex; justify-content: space-between; align-items: center; margin-bottom: -8px; } 
    .unit-name { font-size: 14px; font-weight: 500; color: var(--sys-text-primary); } 
    .compact-chip { transform: scale(0.8); transform-origin: right center; --mdc-chip-label-text-color: var(--sys-primary); } 

    /* Constraints List */ 
    .constraint-item { height: auto !important; align-items: flex-start; padding-top: 12px; } 
    .empty-list-msg { text-align: center; color: var(--sys-text-secondary); font-style: italic; padding: 16px; font-size: 12px; } 
    ...
    .results-content { flex: 1; overflow-y: auto; padding: 0 32px 32px 32px; } 

    /* Viz Layout */ 
    .dashboard-grid-layout { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; padding-bottom: 40px; } 
    .viz-card { 
      background-color: var(--sys-surface); border-radius: 12px; 
      border: 1px solid var(--sys-surface-border); overflow: hidden; 
      display: flex; flex-direction: column; min-height: 320px; 
    } 
    .full-width { grid-column: 1 / -1; min-height: 400px; } 
    .viz-header { 
    ...
    .viz-body { flex: 1; position: relative; padding: 16px; } 
...
export class ScenarioEditorComponent { 
  ...
  readonly totalAllocated = computed(() => { 
    ...
    return res ? res.reduce((acc, curr) => acc + (Number(curr.Patient_Count) || 0), 0).toFixed(0) : '0'; 
  }); 
